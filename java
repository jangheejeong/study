...
ㄴ cpu : cpu에서 연산 및 제어 처리를 하는데 적은 I/O로 많이 처리할 수 있기 때문에 사양이 높을수록 좋음.
메모리가 커야 하드에서 데이터를 많이 읽어 올 수 있고 CPU에서도 메모리 사이즈만큼 데이터를 읽어와 처리할 수 있기 때문에
메모리 사이즈가 크면 적은 I/O발생.
ㄴ 인터페이스 :
ㄴ 추상클래스 : 
ㄴ 객체지향 :
ㄴ di/ioc : 
